...
The two carriers are **not required to be symmetric**. LCP §C.4's own illustration puts `atrHash`
...
Parsing the challenge is the first half. [Paying an x402 seller](/protocols/x402-payment) is the second:
the EIP-3009 authorization whose nonce is the `atrHash` you just verified, and the `PAYMENT-SIGNATURE`
envelope that carries it back.
