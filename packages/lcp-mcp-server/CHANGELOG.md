...
- 64d25d1: The peer floor moves to 0.19.0 — the line, the pins and the pages in one change
...
- Updated dependencies [64d25d1]
  - @integraledger/agentic-terms@0.20.0

## 0.19.0

### Minor Changes

- b8f6856: The peer floor moves to 0.18.3 and the catalog follows it to `zod` 4.6.2 — one line, converged in one change
...
- 4b99f06: The published tarball cites an LCP revision no reader of it can obtain
  
  `0.17.0` is on the registry carrying an INTERNAL SPECIFICATION REVISION beside a section number, in two
  shipped files — `src/annotations.ts:23` and `src/server.ts:16`. That revision is a draft published nowhere
  a reader can reach, so a consumer holding this package meets a citation they cannot follow, in a document
  whose existence and numbering the citation itself discloses.
  
  ⛔ **This entry does not quote the offending string, and that is not squeamishness.** `CHANGELOG.md` is in
  this package's `files`, so a changelog naming the revision would ship the very disclosure it describes.
  `check:spec-citations` refuses exactly that — its subject set is what npm PACKS, derived from each
  manifest's own `files` rather than `src` alone — and it caught this entry's first draft before it could
  reach a tarball.
  ...
  moves without cause.
- Updated dependencies [c94c156]
  - @integraledger/agentic-terms@0.18.0
...
- Updated dependencies
  - @integraledger/agentic-terms@0.17.0
...
- 7c7cb15: Repin the protocol line to 0.16.0, and move the peer floor with it.
...
- Updated dependencies [7c7cb15]
  - @integraledger/agentic-terms@0.15.0
